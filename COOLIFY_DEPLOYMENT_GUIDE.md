nd # Rails 8 Deployment Guide for Coolify
...
```bash
# Database - IMPORTANT: Use DATABASE_URL format (single-database approach)
DATABASE_URL=postgresql://user:password@host:5432/database_production

# Rails Security (generate with: rails secret)
SECRET_KEY_BASE=generate_with_rails_secret
RAILS_MASTER_KEY=your_master_key_from_config_master_key  # If using credentials
...
# Application
APP_DOMAIN=his.doctor
APP_URL=https://his.doctor
...
CLOUDFLARE_R2_BUCKET=hisdoctor-production
...
PORT=80  # CRITICAL: Must be 80 for Traefik routing
RAILS_ENV=production
RAILS_SERVE_STATIC_FILES=true
RAILS_LOG_TO_STDOUT=true
RAILS_LOG_LEVEL=info

# Thruster Configuration (set in Dockerfile, but can override)
...
#### 1.2 Create Solid Table Migrations (CRITICAL - Production 500 Error Fix)

**⚠️ CRITICAL ISSUE**: Rails 8 does NOT automatically create Solid Trifecta tables when using single-database configuration. Missing these tables causes **500 errors in production** when the app tries to access cache/queue/cable.

**Symptoms if you skip this step:**
```
SolidCache::Store::Api#read_serialized_entry failed
PG::UndefinedTable: ERROR: relation "solid_cache_entries" does not exist
```

**The Problem:**
- Rails 8 ships with multi-database configuration for Solid libraries
- When you switch to single-database, the installer doesn't run
- You must manually create migrations for all 3 Solid libraries
- These migrations MUST use `if_not_exists: true` for idempotency

Create three migration files with proper idempotency:

**`db/migrate/YYYYMMDDHHMMSS_create_solid_cache_tables.rb`**:
```ruby
...
class CreateSolidQueue < ActiveRecord::Migration[8.0]
  def change
    ...
    add_index :solid_queue_jobs, [:queue_name, :finished_at],
              name: "index_solid_queue_jobs_for_filtering", if_not_exists: true
    add_index :solid_queue_jobs, [:scheduled_at, :finished_at],
              name: "index_solid_queue_jobs_for_alerting", if_not_exists: true
    add_index :solid_queue_jobs, :class_name, if_not_exists: true
    add_index :solid_queue_jobs, :active_job_id, unique: true, if_not_exists: true
    add_index :solid_queue_jobs, :concurrency_key, if_not_exists: true
    ...
    add_index :solid_queue_scheduled_executions, [:scheduled_at, :priority, :job_id],
              name: "index_solid_queue_scheduled_executions", if_not_exists: true
    ...
    add_index :solid_queue_ready_executions, [:priority, :job_id],
              name: "index_solid_queue_ready_executions", if_not_exists: true
    add_index :solid_queue_ready_executions, :queue_name, if_not_exists: true
    ...
    add_index :solid_queue_blocked_executions, [:expires_at, :concurrency_key],
              name: "index_solid_queue_blocked_executions", if_not_exists: true
    ...
    add_index :solid_queue_failed_executions, [:job_id, :attempts], if_not_exists: true
    ...
    add_index :solid_queue_processes, [:kind, :last_heartbeat_at], if_not_exists: true
    ...
    add_index :solid_queue_semaphores, [:key, :value], unique: true, if_not_exists: true
