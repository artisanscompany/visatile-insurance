...
### Consider WebSocket Updates in Components
...
**React pattern**: Use effect dependencies and ActionCable subscriptions to re-run logic when data changes.

```tsx
useEffect(() => {
  // Re-run when elements change via WebSocket updates
  processElements()
}, [elements])
```
...
9. **Effect dependencies for updates** - Re-run logic when data changes via WebSocket
...
- **PR [#1581](https://github.com/basecamp/fizzy/pull/1581)**: Real-time broadcast conventions
