...
No separate API namespace. Controllers handle both Inertia and JSON:
...
class Cards::ClosuresController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      format.html { redirect_to card_path(@card), notice: "Card closed" }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to card_path(@card), notice: "Card reopened" }
      format.json { head :no_content }
    end
  end
end
...
**Note:** For Inertia pages, redirects work automatically. Inertia intercepts the redirect and performs a client-side navigation. For partial data updates, use Inertia's partial reloads:

```tsx
// React component
router.get(window.location.href, {}, { only: ['cards'] })
```
...
def create
  ...
  respond_to do |format|
    format.html { redirect_to card_path(@card), notice: "Comment added" }
