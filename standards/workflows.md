...
## 5. Real-Time UI Updates with ActionCable + React

**Pattern**: Use ActionCable broadcasts with React state for complex UI state changes.

**Implementation** (from PR [#389](https://github.com/basecamp/fizzy/pull/389)):

```ruby
# Model broadcasts state changes
class Card < ApplicationRecord
  after_update_commit :broadcast_color_change, if: :saved_change_to_color?

  private

  def broadcast_color_change
    ActionCable.server.broadcast(
      "card_#{id}",
      { type: "color_changed", color: color }
    )
  end
end
```
...
```tsx
// React component with real-time color updates
function CardContainer({ card }: { card: Card }) {
  const [cardColor, setCardColor] = useState(card.color)

  useEffect(() => {
    const subscription = consumer.subscriptions.create(
      { channel: 'CardChannel', card_id: card.id },
      {
        received(data: { type: string; color: string }) {
          if (data.type === 'color_changed') {
...
- React state management handles optimistic updates
- Reusable hook pattern across application
...
7. **Real-Time React Updates**: Use ActionCable + React state for complex UI updates
...
- **[#389](https://github.com/basecamp/fizzy/pull/389)**: Stage colors - computed properties, real-time updates
