...
Rails.application.config.after_initialize do
  ...
  Rails::HTML5::SafeListSanitizer.allowed_attributes.merge(%w[ controls type width data-lightbox-target ])
...
class ActionTextRenderingTest < ActionView::TestCase
  test "custom data attributes in content are preserved" do
    html = '<p><img src="photo.jpg" data-lightbox-target="image"></p>'
    content = ActionText::Content.new(html)
    rendered = content.to_s

    assert_match(/data-lightbox-target="image"/, rendered)
  end
end
```
...
## 3. Link Handling in Rich Text Content

**PR**: [#564](https://github.com/basecamp/fizzy/pull/564)

**Pattern**: Handle links in rich text content appropriately based on domain.

**Why it matters**:
- Internal links should use Inertia navigation for SPA-like experience
...
```tsx
// app/views/components/RichTextContent.tsx
...
export function RichTextContent({ html, className }: RichTextContentProps) {
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      // External link - let it open normally in new tab
      ...
      // Internal link - use Inertia navigation
      e.preventDefault()
      router.visit(href)
    }
...
function CardDetail({ card }: { card: Card }) {
  return (
    <article>
      <h1>{card.title}</h1>
      <RichTextContent html={card.description} className="prose" />
    </article>
  )
}
```
...
3. **Use React components for client-side enhancements** like link handling
