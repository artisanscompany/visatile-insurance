...
Command::Result::ShowModal = Struct.new(:modal_component, :props)
...
def respond_with_execution_result(result)
  ...
  when Command::Result::ShowModal
    render json: { modal: result.modal_component, props: result.props },
           status: :accepted
  else
...
**Why `ShowModal`?**: "Fizzy Ask" opens a chat modal where users can have a conversation with an LLM. The command parser returns a `ShowModal` result, and the controller renders JSON that tells the React frontend which modal component to display.
